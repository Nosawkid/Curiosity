    ...
    <div style={{ padding: "20px" }} className='mylearning'>
      ...
      <Box sx={{display:"flex",alignItems:"center",gap:"20px",flexWrap:"nowrap",justifyContent:"center"}}>
      {
        courses && courses.length > 0 ? courses.map((row, key) => (
          <>
          {
            row.courseId ?  <Card  onClick = {() =>viewCourse(row.courseId._id)} className='courseCard' sx={{ width: "300px", height: "350px" }}>
          <CardContent sx={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            <img style={{ width: "100%", height: "200px", objectFit: "cover" }} src={row.courseId.courseImage || Courseholder} alt="Cover" />
            <Typography sx={{ fontSize: "18px", fontWeight: "bold" }} variant='h3'>{row.courseId.courseTitle && row.courseId.courseTitle}</Typography>
